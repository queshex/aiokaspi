...
class ConfigManager:
    ...
    def get_lazy_config(self) -> Config:
        device: dict | None = self.storage.get(Entity.device)
        if device is None:
        ...
        keys: dict | None = self.storage.get(Entity.keys)
        if keys is None:
            public_key, private_key = KeysGenerator.generate_keypair_base64()
            keys: dict = asdict(
                KeysSchema(public_key=public_key, private_key=private_key)
            )
            self.storage.save(entity=Entity.keys, data=keys)

        session: dict | None = self.storage.get(Entity.session)
        extra: dict | None = self.storage.get(Entity.extra)

        pk: str = KeysGenerator.get_pk(keys["public_key"])
        pk_tag: str = KeysGenerator.get_pk_tag(keys["public_key"])
        return Config.from_sources(
            pk=pk, pk_tag=pk_tag, device=device, keys=keys, session=session, extra=extra
        )
